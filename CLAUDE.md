...
> **当前状态**：503 实体 / 18 论点 / 0 unknown / 全部验证通过（2026-09-13）
...
|------|:--:|
| 产业 | 2 |
| 赛道 | 67（唯一数）· **双产业归属**：44 个出现在「AI算力」视图、39 个在「半导体」视图，其中 **16 个双归属**（见下） |
| 公司 | 405（2026-09-13：先去重 12 个重复条目 385→373，补建 24 家缺页标的 →397，再补建 9 家赛道成员 →406；同日确认 Cabot Microelectronics 已并入 Entegris，删除其当前主体页 →405） |
| 概念卡片 | 10 |
| 投资论点 | 18 |
| 总词条 | 502（2产业 + 67赛道 + 405公司 + 10概念 + 18论点） |
| QA 版本 | v1.3 |

> **产业归属模型（2026-09-13 起）**：赛道用 `industries` **列表**（可多值），公司用 `industries`（由 segments 派生的并集）。
> 编译期派生 `industry`（= 列表首位）供显示/搜索用，前端筛选走 `industries`（`inIndex()`）。
> **16 个双归属赛道**（同时服务两条产业链，在「AI算力」与「半导体」两个视图都出现）：
> `光刻机` `电子特气` `高纯硅料与硅片` `溅射靶材` `刻蚀设备` `薄膜沉积设备` `封装基板材料`
> `晶圆代工(先进制程)` `检测量测设备` `光刻胶与湿化学品` `CPU(服务器级)` `FPGA`
> `Chiplet与异构集成` `RISC-V AI芯片` `EDA与IP核` `高速连接器与铜缆`
>
> 历史背景：这 16 个赛道原先在「半导体」与「AI算力」目录各存一份独立文件（共 81 文件），
> 因各自演进而内容分叉；2026-09-13 合并为单文件双归属，消除了重复节点与矛盾数字。
...
> **Tier 3 的可执行清单**：`docs/待深化龙头清单.md`
> —— 从「被赛道定位为龙头级」的公司里筛出深度未达 NVIDIA 那一档的，
> 分「🔴 薄（13 家，字段都没填齐）」与「⚠️ 中等（32 家，只需展开叙事）」两档。
> 由 `L1-Schema与Pipeline/lint/list_deepening_targets.py` 生成，数据变了重跑即可。
...
- 深度覆盖公司全部添加 `data_freshness_date` 字段（Phase 3完成时78家 → 2026-09-13 骨架补全/D1-D5批次后 **198家**）
- ⚠️ 保鲜监控覆盖范围为这 198 家；其余 **207 家**公司无 `data_freshness_date`，不纳入扫描
