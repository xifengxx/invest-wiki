...
- company: 数据中心/云服务商
  supplies: 云计算基础设施
  note: Cadence Cloud SaaS部署
customers:
- company: 全球TOP 30半导体公司
  note: 几乎每颗AI芯片设计都同时使用Cadence和Synopsys工具
- company: NVIDIA
  ticker: NVDA
  note: ChipStack AI Agent联合开发+GPU优化EDA
- company: 台积电/三星/Intel
  note: N3/N2/A16/A14先进节点认证工具链
- company: MediaTek/Qualcomm/Apple/AMD
  note: 移动SoC+AI芯片设计全流程
partners:
- company: NVIDIA
  ticker: NVDA
  area: Agentic AI芯片设计
  note: ChipStack AI Super Agent联合开发
- company: Google Cloud
  area: 云端EDA优化
  note: ChipStack云端部署+优化
competitors:
- company: Synopsys
  ticker: SNPS
  area: EDA工具链
  note: 双寡头，全球EDA#1（~31%份额+Ansys）
- company: Siemens EDA (Mentor)
  area: EDA工具
  note: 排名#3，PCB/系统设计强势
core_business:
- 数字IC设计与签核（Innovus/Genus/Tempus，~30% EDA市场份额）
- 定制/模拟IC设计（Virtuoso/Spectre，模拟设计行业标准）
- 功能验证（Xcelium逻辑仿真/Jasper形式验证/Verisium AI验证）
- 硬件仿真与原型验证（Palladium Z3/Protium）
- 半导体IP核（Star IP系列，~14%营收，+22% YoY）
- 系统设计与分析（Sigrity/Clarity，~16%营收，+18% YoY，Hexagon收购多物理场仿真）
...
全球EDA双寡头#2（~30%市场份额），Virtuoso模拟设计行业标准30年，NVIDIA合作Agentic AI芯片设计平台ChipStack——AI Agent自主执行设计流程，积压$8.0B创新高，Rule of 60首次实现。
...
| 板块 | 占比 | YoY增长 | 核心产品 |
|------|:--:|:--:|------|
| **EDA工具** | ~70% | — | Innovus/Virtuoso/Xcelium/Palladium |
| **IP核** | ~14% | +22% | Star IP系列（PCIe/HBM/USB/DDR） |
| **系统分析** | ~16% | +18% | Sigrity/Clarity + Hexagon多物理场 |
...
|------|:--:|:--:|
| EDA市场份额 | ~31% | **~30%** |
| 收购策略 | Ansys（$35B，系统仿真） | Hexagon（多物理场仿真） |
| 数字IC优势 | Fusion Compiler | Innovus/Genus |
| 模拟IC优势 | PrimeSim | **Virtuoso（行业标准）** |
