...
# ✅ 正确（保留原文 + 换行追加，使用 YAML | 块标量保留换行）
one_liner: |
  全球HBM存储龙头(市占~56%)，专注AI用HBM/DRAM/NAND全栈内存芯片，通过高端AI存储溢价及长期供应协议实现盈利，位于半导体IDM制造中游——HBM占营收42%且产能售罄至2026全年。
  【2026.7.29更新】Q2营收79万亿(miss)，HBM4 Q2出货+HBM4E送样，LTA首次确认含保证金，PE 4x历史底部。
```

> **格式要求**：使用 YAML `|` 块标量（block scalar），原文和新内容各占一行。渲染时 `white-space: pre-line` 会将 `\n` 显示为换行。
...
- **未查重就新建赛道**（2026-09-13 新增）——新建赛道前必须按 `name` 与 `slug` 全库搜索；同义赛道已存在于另一产业时，优先决定归属并**复用**，不得各建一份。历史上曾因此产生 16 组跨产业重复档案（`semi-X.md` ↔ `X.md`），清理需逐条合并 58 条趋势/34 条证据/61 个标的
- **新建赛道只建档、不接入图谱**（2026-09-13 新增）——必须同时补齐 `wikilinks`≥3、`companies`≥3、`key_inputs`/`key_customers` 非空，并在被引用方反向登记 + 更新 `index.md`，否则该赛道 backlink 恒为 0、在可视化图谱中孤立
- **在 YAML frontmatter 中使用 Markdown 加粗**（2026-09-13 新增）——`- **xxx**` 或 `field: **xxx**` 中的首个 `*` 会被当作 YAML alias，导致**整个 frontmatter 解析失败**、实体退化为 `unknown`。body 段内不受限。编译后 `unknown:` 计数必须为 0
- **未按 ticker 查重就新建或升级公司词条**（2026-09-13 新增）——同一公司曾出现两个深度词条（`jcet.md` 与 `jcap.md`，ticker 一个对 `600584.SH` 一个错 `600584.SS`），起因是升级骨架时未先查是否已有同名/同代码条目。**新建或升级公司词条前必须先跑 ticker 查重**（脚本见 lint 规范）
- **编译后不检查图谱完整性**（2026-09-13 新增）——`engine/graph.py` 曾因单遍扫描建边**静默丢掉 51% 的边**且不报错；且赛道页用纯文本列标的而非 wikilink，若不合成赛道→公司边，全部公司节点孤立。**每次编译后必须检查**：边:节点比 > 1.5、公司节点有入边 > 70%、孤立赛道 = 0、`unknown` = 0（脚本见 lint 规范）

> 以上各项的自动检测脚本见 `lint/执行指令-定期扫描.md`
